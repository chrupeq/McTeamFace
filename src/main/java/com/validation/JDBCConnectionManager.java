...
//		try {
//			networkEntityArray = (Base_data[]) ReadDataSetIntoMainMemory.readFileInFromHardDrive(path);
	...
	public void sendBaseDataToDB(NetworkEntity[] networkEntityArray) {
		...
		for (int i = 0; i < networkEntityArray.length; i++) {
			...
			try {
				...
				String s = formatter.format(b.getDate_time().getTime());
				statement.executeUpdate("INSERT INTO base_data(date_time, event_id, failure_class, ue_type, market, operator, cell_id, duration, cause_code, ne_version, imsi, hier3_id, hier32_id, hier321_id)VALUES('" + s + "',"
