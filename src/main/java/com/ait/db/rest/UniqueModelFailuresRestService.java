...
public class UniqueModelFailuresRestService {
	...
	@Path("/{ue_type}")
	public Response getAllUniqueDataForModel(@PathParam("ue_type") final int tacNumber) {
		try{
			System.out.println(tacNumber);
			final List<Base_data> modelList = UniqueModelFailuresDao.getAllUniqueModels(tacNumber);
			if(modelList.isEmpty()) {
				return Response.status(404).build();
			}
			final ObjectMapper mapper = new ObjectMapper();
			final String jsonInString = mapper.writeValueAsString(modelList);
	...
	@Produces({MediaType.APPLICATION_JSON})
	public Response getUniqueModelsBetweenDates(@QueryParam("tacNumber")final int tacNumber, @QueryParam("dateOne") String dateOne, @QueryParam("dateTwo") String dateTwo) {
		...
		try {
			final List<Base_data> baseDataList = UniqueModelFailuresDao.getAllUniqueModelsBetweenDates(tacNumber, dateOne, dateTwo);
			if(baseDataList.isEmpty()) {
				return Response.status(404).build();
			}
			final ObjectMapper mapper = new ObjectMapper();
			final String jsonInString = mapper.writeValueAsString(baseDataList);
