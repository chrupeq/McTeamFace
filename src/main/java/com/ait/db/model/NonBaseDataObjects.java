...
public class NonBaseDataObjects {
	...
	public static Failure_class[] createFailureClass(Object[][] array){
		...
		for(int i=0; i <= array.length; i++){
			...
			fc.setDescription(array[i][0].toString());
			fc.setFailure_class(Integer.parseInt(array[i][1].toString()));
	...
	public static User_equipment[] createUserEquipment(Object[][] array){
		...
		for(int i=0; i <= array.length; i++){
			...
			ue.setUser_equipment_type(array[i][5].toString());
			ue.setOperating_system(array[i][6].toString());
			ue.setInput_mode(array[i][7].toString());
